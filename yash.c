...
void handle_ctrl_c(int sig)
{
    ...
    if (job_count > 0)
    {
        kill(-jobs[top_stack_job()].pgid, SIGKILL);
        while (waitpid(-jobs[top_stack_job()].pgid, &status, 0) != -1) {
        // Got a child.  Wait for more.
        }
        free(jobs[top_stack_job()].cmd);
        job_count--;
    }
...
void handle_ctrl_z(int sig)
{
    if (job_count > 0)
    {
        kill(jobs[top_stack_job()].pgid, SIGTSTP);
...
void handle_fg_cmd()
{
    ...
    if (job_count > 0)
    {
        ...
        printf("%s", jobs[top_stack_job()].cmd);
...
void periodic_reap_for_jobs()
{
    int index, status;
    for (index = 0; index < job_count; index++)
    {
        waitpid(-jobs[index].pgid, &status, WNOHANG);
    ...
    while(cmdline = readline("# "))
    {
        ...
        else
        {
            ...
            if (amp_index == NONE)
            {
                ...
                if (WEXITSTATUS(status) == 0 | WEXITSTATUS(status) == 255)
                {
                    if (jobs[top_stack_job()].cmd != NULL)
                    {
                        free(jobs[top_stack_job()].cmd);
                    }
                    job_count--;
        ...
        periodic_reap_for_jobs();


        // printf("Top Stack Job: %d, %d, %d\n",  jobs[top_stack_job(job_count)].pgid,  jobs[top_stack_job(job_count)].p1.pid,  jobs[top_stack_job(job_count)].p2.pid);
        printf("Wait Status: %d, PGID: %d\n", WEXITSTATUS(status), retcpid);
        // printf("Job Count: %d\n", job_count);
        // for (int i = 0; i < job_count; i++)
        // {
        //     printf("%d. pgid: %d, p1id: %d, p2id: %d\n", i, jobs[i].pgid, jobs[i].p1.pid, jobs[i].p2.pid);
        // }
